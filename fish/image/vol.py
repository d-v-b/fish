...
class InterpArray:
    ...
    def __repr__(self):
        return f'An interpolated array with size {self.full_shape} sampled' \
               f' at {self.y} along axis {self.interpolation_axis}'
    ...
    def _get_interpolated_value(self, idx):
        from numpy import where, diff, array, tensordot
        ...
        dists = self.y.reshape(-1, 1) - idx_interp
        result = []
        ...
        for d_ in dists.T:
            ...
            if ds_ind_pre == -1:
                idx_inner[ipax] = 0
                result.append(self.x[tuple(idx_inner)])
                continue
            ...
            x_ = self.x[tuple(idx_inner)]

            new_coeff_shape = [1] * x_.ndim
            new_coeff_shape[keep_axes.index(ipax)] = coeffs.size

            # perform linear interpolation
            result.append((coeffs.reshape(new_coeff_shape) * x_).sum(keep_axes.index(ipax)))
